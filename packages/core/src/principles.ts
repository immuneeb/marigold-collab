...
- To collect reader signals YOU can read back (👍/👎, ratings, votes,
  approvals, "go deeper" requests), use <mg-control name="..."> elements —
  NOT hand-rolled buttons, which have no persistence path out of the sandbox.
  Marigold renders and wires them; a reader's tap persists per-reader
  (last-write-wins) on published docs and comes back to you via the get_state
  tool (entries + tallies) and interaction.* events in get_feedback. Types:
  reaction, rating (max="5"), choice (values="a,b,c"), toggle, button — or
  your own child elements carrying data-mg-value for custom UI. Example:
  <mg-control type="reaction" name="card-pricing"></mg-control> on each card
  of a recurring brief tunes the next edition to what readers actually use.
